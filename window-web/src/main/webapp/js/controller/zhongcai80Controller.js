app.controller('zhongcai80Controller', function($scope,$controller, zhongcai80Service) {
	...
	$scope.save = function() {
		...
		zhongcai80Service.add($scope.entity).success(function(response) {
			if (response.success) {
				$scope.reloadList();
			} else {
	...
	$scope.search = function(pageNum, pageSize) {
		zhongcai80Service.search(pageNum, pageSize, $scope.searchEntity).success(
				function(response) {
