app.controller('xinzhongcai80Controller', function($scope,$controller, xinzhongcai80Service) {
	...
	$scope.save = function() {
		...
		xinzhongcai80Service.add($scope.entity).success(function(response) {
			if (response.success) {
				$scope.reloadList();
			} else {
	...
	$scope.search = function(pageNum, pageSize) {
		xinzhongcai80Service.search(pageNum, pageSize, $scope.searchEntity).success(
				function(response) {
