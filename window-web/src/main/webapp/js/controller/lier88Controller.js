app.controller('lier88Controller', function($scope,$controller, lier88Service) {
	...
	$scope.save = function() {
		...
		lier88Service.add($scope.entity).success(function(response) {
			if (response.success) {
				$scope.reloadList();
			} else {
	...
	$scope.search = function(pageNum, pageSize) {
		lier88Service.search(pageNum, pageSize, $scope.searchEntity).success(
				function(response) {
