app.controller('demasi88Controller', function($scope,$controller, demasi88Service) {
	...
	$scope.save = function() {
		...
		demasi88Service.add($scope.entity).success(function(response) {
			if (response.success) {
				$scope.reloadList();
			} else {
	...
	$scope.search = function(pageNum, pageSize) {
		demasi88Service.search(pageNum, pageSize, $scope.searchEntity).success(
				function(response) {
